  ...
  <div style="margin-top:10px;">
    ...
    <div v-if="tabType == 0">
      <div class="leftdatalis">
        <ul class="detalis">
          <li>
            <span>水表序列号：</span>
            <strong>{{allDatas.uuid}}</strong>
          </li>
          <li class="threeName">
            <span>房源名称：</span>
            <strong>{{allDatas.homeName}}</strong>
            <span>楼层：</span>
            <strong>{{allDatas.floorName}}</strong>
            <span>房间号：</span>
            <strong>{{allDatas.roomName}}</strong>
          </li>
          <li>
            <span>产品类型：</span>
          ...
          <li>
            <span>绑定时间：</span>
            <strong>{{allDatas.createdAt}}</strong>
          </li>
          <li>
            <span>更新时间：</span>
            <strong>{{allDatas.meterUpdatedAt}}</strong>
          </li>
          <li>
            <span>水表读数：</span>
          ...
          <li>
            <span>当月累计水表量：</span>
            <strong style="color:red">{{allDatas.meterAmount}}</strong>
          </li>
          <li>
            <span>通讯状态：</span>
    ...
    <div v-if="tabType == 1">
      ...
      <div class="table">
        ...
        <el-table :data="ExportDatas" border style="width: 100%; height:50%">
          <el-table-column prop="smartWatermeterId" label="水表序列号"></el-table-column>
          <el-table-column prop="createdAt" label="时间"></el-table-column>
          <el-table-column prop="deviceAmount" label="水表读数(吨)"></el-table-column>
          <el-table-column prop="dayAmount" label="增量读数(吨)"></el-table-column>
        </el-table>
        <el-pagination
    ...
    <div v-if="tabType == 2">
      <el-table :data="exceptionDatas" border style="width: 100%; height:50%">
        <el-table-column prop="apartmentName" label="时间"></el-table-column>
        <el-table-column prop="onoffStatus" label="通讯状态">
  ...
  methods: {
    ...
    getWaterDetails(val) {
      ...
      $.ajax({
        ...
        if (val == 1) {
          this.ExportDatas =
            res.dataResponseBodyVO.dt.watermeterRecordManagerVOList.list;
          this.page.total =
            res.dataResponseBodyVO.dt.watermeterRecordManagerVOList.total;
        }
        if (val == 2) {
          //异常记录
    ...
    formatDate(time) {
      ...
      return (
        tmpDate.getFullYear() +
        "-" +
        String(tmpDate.getMonth() + 1) +
        "-" +
        String(tmpDate.getDate())
      );
    },
    //点击页数
